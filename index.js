...
function isAlmostSame(num1, num2) {
  ...
  if (isAlmostSame = (num1 - num2 < Number.EPSILON && num2 - num1 < Number.EPSILON));
  return isAlmostSame;
}
...
function isSafeInteger(num) {
  ...
  if (isSafeInteger <= num.MAX_SAFE_INTEGER);
  if (isSafeInteger >= num.MIN_SAFE_INTEGER);
  return num <= num.MAX_SAFE_INTEGER && num >= num.MIN_SAFE_INTEGER;
}
...
function checkIsInteger(num) {
  ...
  if (checkIsInteger = (Number.isInteger(num)));
...
function checkIsSafeInteger(num) {
  ...
  if (checkIsSafeInteger = (Number.isSafeInteger(num)));
  return checkIsSafeInteger;
}
